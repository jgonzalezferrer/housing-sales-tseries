...
As the new p-value shows, there is no evidence to reject the null hypothesis now. Therefore, residuals can indeed be considered normal if this outsider is not taken into account.


### Improving ARIMA models

The ARIMA model we have until now has some upsides, in the sense that a) it is simple parameter-wise, b) was obtained following the theory and c) residuals meet most of the assumptions. Independence of these, however, has not been achieved. Because of this fact, and because ARIMA models are hard to fit, and there is no way to be sure that the most appropriate one has been found, a couple alternative methods for parameter tuning have been tried next. 


#### Alternative 1: Improving based on RMSE

A reasonable approach is to try and find a model which offers a lower RMSE than the one we have until now (roughly, for our first model, $RMSE = 0.152$). Along this line, maintaining the already calculated parameters, we tuned them increasing their values one by one.The result was the following model, an $ARIMA(2,1,1)(1,1,1)_{12}$.
...
# Find the outsider and remove it
which.max(model1$residuals)
jarque.bera.test(model1$residuals[-117])
```

Assumptions-wise this model performs better than the previous one. While residuals still do not have a mean of zero, results of the Box-Pierce and Jarque-Bera tests (the latter after ruling out the outsider at position 117) show that they are independent and normal. Moreover, root mean squared error has been reduced, achieving a slightly lower $RMSE = 0.146$, and AIC is also somewhat lower. The downside of this model is complexity, as it uses more complex parameters than the previous one, and the gain in terms of assumptions or RMSE is arguably not significant enough to make up for that fact.

#### Alternative 2: Improving based on hypothesis-meeting

This time, priority has been given to finding a model which fullfills all the hypothesis, that is, one which guarantees that residuals are independent, normal, and have zero mean. Within this subset of models, we have chosen the one with lowest RMSE: an $ARIMA(1,2,2)(1,0,2)_{12}$.
...
While, according to the test, the mean is not equal to zero, further checking the results shows that it is close enough for us to consider it zero, $9.8*10^-4$.

```{r alt_models_2b}
Box.test(model2$residuals, lag=24, fitdf=6, type="L")
jarque.bera.test(model2$residuals)

# Again, find and remove the outlier
which.max(model2$residuals)
jarque.bera.test(model2$residuals[-117])
```

All the assumptions are now met, with the residuals also being normal and independent. AIC and RMSE are also the lowest of all models. Surprisingly, this model contradicts the theoretical one in the sense that it does not include any seasonal differences, but in exchange it does feature seasonal AR 1 and MA 2.

#### Alternative 3: auto.arima()

Package forecast comes with a function to automatically find the best ARIMA model. This function, however, is not very trustable, and its results have to be tested. The result is an $ARIMA(0,2,2)(0,0,2)_{12}$.
...
# Remove the outsider and test again
which.max(model3$residuals)
jarque.bera.test(model3$residuals[-117])
```

As seen above, the resulting model does have independent residuals, and their mean is very close to zero. However they are not normal, not even after removing the outsider. In addition, RMSE has increased from previous iterations, and AIC has increased drastically. All in all, there are better alternatives than this automatically fitted ARIMA.
