...
A seasonal random walk is defined as $\hat{Y}_t = Y_{t-12} + \mu$, where $\mu$ is the average annnual trend. In this case, there is not evidence to reject the null hypothesis in the <i>adf.test()</i> and besides the ACF plot still has many positive autocorrelations. As we have already seen, this is an evidence to apply one order of non-seasonal difference.
...
Let us study what values of differencing the functions <i>ndiffs</i> and <i>nsdiffs</i> suggest. These results should be taken into account with a grain of salt and just use them to support our analysis.
```{r}
...
Surprisingly, depending on the test we use to estimate the number of differences required to make the time series stationary, we obtain a different value. This extra differencing order is a particular case where the KPPS test detects that the first order difference of the time series is still not stationary but trend-stationary, and another order of differencing is needed. In contrast, the ADF test rejects the null hypothesis of present of a unit root against a stationary root alternative. On the other hand, <i>nsdiffs</i> indicates that we do not need any order of seasonal differencing.
...
The remanining residuals look nice though the presence of the some autocorrelation. These peaks does not mean that the model is wrong because even in simulated time series model they might appear. The obtained $ARIMA(0,1,1)(0,1,1)_{12}$ model is essentially a "seasonal exponential smoothing model", which equation is:
